...
interface UnoSetupProps {
  onBack: () => void;
  onRoomJoined: (roomCode: string, playerId: string, playerName: string) => void;
}

export const UnoSetup: React.FC<UnoSetupProps> = ({ onBack, onRoomJoined }) => {
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      ...
      const success = await createUnoRoom(code, playerId, playerName.trim());
      if (success) {
  ...
  return (
    <div className="glass-panel max-width-container animated-slide-in text-center" style={{ padding: "30px 20px" }}>
      ...
      <div className="help-box" style={{ marginTop: "32px", fontSize: "0.8rem" }}>
        ...
        <p style={{ color: "#a1a1aa", lineHeight: "1.4", textAlign: "left" }}>
          Uno Trực Tuyến đồng bộ các người chơi trong cùng một phòng qua máy chủ kết nối trực tuyến. Để chơi chung, 1 người chơi tạo phòng và gửi mã 4 chữ cái cho các người chơi khác nhập để cùng tham gia.
        </p>
      </div>
            ...
            {showFirebaseGuide && (
              <div style={{
                ...
                <ol style={{ paddingLeft: "16px", display: "flex", flexDirection: "column", gap: "4px" }}>
                  ...
                  <li>Tạo <b>Realtime Database</b> trong menu trái.</li>
                  <li>Tại tab <b>Rules</b> của Database, đặt: <code style={{ color: "#fda4af" }}>{`".read": true, ".write": true`}</code> để chơi trực tiếp.</li>
