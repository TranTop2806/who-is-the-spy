...
import { initializeGameDeck } from "../../data/explodingCards";
import { ref, set } from "firebase/database";

interface ExplodingLobbyProps {
  roomCode: string;
  playerId: string;
  playerName: string;
  onGameStarted: () => void;
...
export const ExplodingLobby: React.FC<ExplodingLobbyProps> = ({
  roomCode,
  playerId,
  playerName,
  onGameStarted,
  ...
  useEffect(() => {
    ...
    const unsubscribe = listenToRoom(roomCode, (data) => {
      ...
      // If room status changed to PLAYING, trigger the callback
  ...
  const handleLeaveRoom = async () => {
    if (window.confirm("Bạn có chắc chắn muốn rời phòng?")) {
      ...
      try {
        ...
        if (isHost) {
          // If host leaves, we terminate the room
  ...
  const handleStartGame = async () => {
    ...
    try {
      // Initialize the deck and deal cards to players
      const { playerHands, drawPile } = initializeGameDeck(playerIds);
      ...
      const updates: any = {
        ...
        logs: [...(roomData.logs || []), `🎮 Trận đấu đã bắt đầu! Đến lượt ${players[playerIds[0]].name}`]
      };

      // Set the hands for all players
      ...
      <div className="form-group" style={{ marginBottom: "32px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
          <span className="input-label" style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <Users size={18} /> Danh sách người chơi ({playerIds.length}/8)
          </span>
        </div>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  border: isMe ? "1px solid rgba(99, 102, 241, 0.2)" : "1px solid rgba(255, 255, 255, 0.05)",
                  borderRadius: "12px"
                }}
              >
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  <div style={{
                    ...
                    height: "32px",
                    borderRadius: "50%",
                    background: isMe ? "linear-gradient(135deg, var(--color-primary) 0%, var(--color-secondary) 100%)" : "rgba(255, 255, 255, 0.08)",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    fontSize: "0.85rem",
